      ...
      <td class="align-middle">
        <div class="d-flex flex-row">
          ...
          <input min="0" name="quantity" value="${
            product.quantity || 1
          }" type="number" class="form-control form-control-sm quantity-input no-edit" style="width: 50px;" readonly>
...
function calculateTotals() {
  ...
  document.querySelectorAll('.usd-total').forEach((element) => {
    element.textContent = `USD ${totalUSD}`
  })
  document.querySelectorAll('.uyu-total').forEach((element) => {
    element.textContent = `UYU ${totalUYU}`
  })
...
function calculateShippingCost(totalConvertedUSD, totalUYU) {
  ...
  document.querySelector(
    '.shipping-cost'
  ).textContent = `UYU ${shippingCost.toFixed(0)}`
  ...
  document.querySelectorAll('.grand-total').forEach((element) => {
    element.textContent = `UYU ${grandTotal.toFixed(0)}`
  })
}
      ...
      if (result.isConfirmed) {
        ...
        Swal.fire({
          ...
          const paymentModal = bootstrap.Modal.getInstance(
            document.getElementById('modalPaymentOptions')
          )
          if (paymentModal) {
            paymentModal.hide()
          } //Cierra el modal
...
function openPaymentOptions() {
  if (validateAddressAndShipping()) {
    ...
    const shippingModal = bootstrap.Modal.getInstance(
      document.getElementById('modalShippingOptions')
    )
    if (shippingModal) {
...
function validateAddressFields() {
  ...
  document.querySelector('#modalShippingOptions .btn-primary').disabled =
    !allFieldsFilled
}
...
function validateAddressAndShipping() {
  ...
  const shippingSelected = document.querySelector(
    'input[name="shipping-type"]:checked'
  )
  if (!shippingSelected) {
