  ...
  sellForm.addEventListener('submit', function (e) {
    ...
    if (!infoMissing) {
      ...
      getJSONData(PUBLISH_PRODUCT_URL).then(function (resultObj) {
        // Obtenemos el mensaje que viene de la API
        let msgToShow = resultObj.data?.msg || 'Mensaje no especificado'

        if (resultObj.status === 'ok') {
          // SweetAlert2 para mostrar mensaje de éxito
          Swal.fire({
            title: '¡Éxito!',
            text: msgToShow, // Mensaje de éxito de la API
            icon: 'success',
            confirmButtonText: 'Aceptar'
          })
        } else if (resultObj.status === 'error') {
          // SweetAlert2 para mostrar mensaje de error
          Swal.fire({
            title: 'Error',
            text: msgToShow, // Mensaje de error de la API
            icon: 'error',
            confirmButtonText: 'Aceptar'
          })
        }
